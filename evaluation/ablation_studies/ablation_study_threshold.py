...
def run_ablation(config_name_or_path: Union[CfgNode, str], dataset: str) -> pd.DataFrame:
    ...
    for model, output_dir, data_module in zip(models, output_dirs, data_modules):
        ...
        for threshold in [0.90, 0.95, 0.99, 1.0]:
            adt_estimator = DistanceChecker(output_dir, quantile=threshold)
            ...
            for (name_eval_loader, eval_loader) in data_module.get_all_ood_dataloaders():
                ...
                current_metrics_dict["quantile"] = threshold
                ...
                print(
                    tabulate(
                        current_metrics[["dataset", "accuracy", "quantile"] + error_cols],
                        headers="keys",
                    )
                )

    if len(metrics) != 0:
        metrics.to_csv(save_dir / "metrics_quantiles.csv")
    return metrics
...
if __name__ == "__main__":
    ...
    aggregated_metrics = (
        all_metrics[error_cols + ["quantile"]].groupby(by=["quantile"]).aggregate(func=["mean", "std"])
    )
    print(tabulate(aggregated_metrics, headers="keys"))
    all_metrics.to_csv(f"outputs/{args.dataset}/ablation_q_all.csv")
    aggregated_metrics.to_csv(f"outputs/{args.dataset}/ablation_q_aggregated.csv")
